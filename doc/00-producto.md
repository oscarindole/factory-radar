...
El planteamiento inicial llama **FACTORY RADAR** a la vez a la plataforma
completa y al primer módulo (el de producción). Eso rompe en cuanto se dibuja el
menú lateral: no puede haber un "Factory Radar" dentro de "Factory Radar".

**Decisión.** La plataforma es **FACTORY RADAR**. El módulo de producción pasa a
llamarse **PRODUCTION RADAR**. La familia queda:

```
FACTORY RADAR  (la plataforma)
